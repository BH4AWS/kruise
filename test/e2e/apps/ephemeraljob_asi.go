...
var _ = SIGDescribe("ephemeraljob-asi", func() {
	f := framework.NewDefaultFramework("ehpemeraljobs-asi")
	var ns string
	...
	framework.KruiseDescribe("EphemeralJob Creating", func() {
		...
		ginkgo.It("create ephemeral running err command job", func() {
			...
			ginkgo.By("Create EphemeralJob job-" + randStr)
			job := tester.CreateTestEphemeralJob(randStr, 6, 6, metav1.LabelSelector{
				MatchLabels: map[string]string{
					"run": "nginx",
				}}, []v1.EphemeralContainer{
				{
			...
			}, 60*time.Second, 10*time.Second).Should(gomega.Equal(int32(6)))
		})
	...
	framework.KruiseDescribe("Create Ephemeral Container", func() {
		...
		ginkgo.It("test ephemeral container in in-place-update", func() {
			...
			ginkgo.By("Update image to reg.docker.alibaba-inc.com/base/nginx")
