...
type realPuller struct {
	...
	secretManager util.SecretManager
...
func newRealPuller(runtime imageruntime.ImageRuntime, secretManager util.SecretManager, eventRecorder record.EventRecorder) (*realPuller, error) {
...
func newRealWorkerPool(name string, runtime imageruntime.ImageRuntime, secretManager util.SecretManager, eventRecorder record.EventRecorder) *realWorkerPool {
	w := &realWorkerPool{
		name:          name,
		runtime:       runtime,
		secretManager: secretManager,
...
func newPullWorker(name string, tagSpec appsv1alpha1.ImageTagSpec, secrets []v1.Secret, runtime imageruntime.ImageRuntime, statusUpdater imageStatusUpdater, ref *v1.ObjectReference, eventRecorder record.EventRecorder) *pullWorker {
	o := &pullWorker{
		name:          name,
		tagSpec:       tagSpec,
		secrets:       secrets,
...
func (w *pullWorker) Run() {
	...
	defer func() {
		cost := time.Now().Sub(startTime.Time)
...
func (w *pullWorker) getImageInfo(ctx context.Context) (*imageruntime.ImageInfo, error) {
