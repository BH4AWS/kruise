...
func NewContainerdImageRuntime(
	...
	accountManager util.ImagePullAccountManager,
) (ImageRuntime, error) {
...
func (d *containerdImageClient) getResolver(ctx context.Context, ref reference.Named, secrets []v1.Secret) (_ remotes.Resolver, isSchema1 bool, _ error) {
	var registry = util.ParseRegistry(ref.String())
	...
	// anonymous user.
	authInfos := make([]*util.AuthInfo, 0, 2)
...
func (d *containerdImageClient) resolverGenerator(authInfo *util.AuthInfo) remotes.Resolver {
	var username string
	var secret string
