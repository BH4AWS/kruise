...
import (
	...
	jsonutil "github.com/openkruise/kruise/pkg/util"
)
...
func convertToRegistryAuthInfo(secret v1.Secret, registry string) (*util.AuthInfo, error) {
	auths := Auths{}
	if secret.Type == v1.SecretTypeOpaque {
		return &util.AuthInfo{
...
func containsImage(c []ImageInfo, name string, tag string) bool {
	for _, info := range c {
		for _, repoTag := range info.RepoTags {
			imageRepo, imageTag := util.ParseRepositoryTag(repoTag)
...
func (r *imagePullStatusReader) mainloop() {
	...
	for {
		...
		default:
			...
			klog.V(5).Infof("runtime read progress %v", jsonutil.DumpJSON(jm))
			if jm.ID != "" {
			...
			r.seedPullStatus(ImagePullStatus{Process: int(currentProgress), DetailInfo: jsonutil.DumpJSON(progress)})
		}
	}
}
